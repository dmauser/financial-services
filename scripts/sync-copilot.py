...
def sync_copilot_marketplace(dest_dir: Path | None = None) -> int:
    """Write the repo-root .claude-plugin/marketplace.json as a Copilot-dedicated
    marketplace registering only the umbrella `financial-services` plugin.

    Why .claude-plugin/ and not .copilot-plugin/?  Empirically (verified 2026-06-14),
    Copilot CLI's `marketplace add <owner>/<repo>` ONLY reads .claude-plugin/marketplace.json
    at the repo root - .copilot-plugin/ is ignored. To make the umbrella installable
    via the repo-URL shorthand we have to live in that file. Upstream's per-specialist
    20-plugin marketplace is intentionally replaced; users who want the upstream Claude
    plugins should install from `anthropics/financial-services` directly.

    Install (one command after marketplace add):

        copilot plugin marketplace add dmauser/financial-services
        copilot plugin install financial-services@financial-services-copilot

    `dest_dir` overrides the output directory. Used by check.py for drift detection.
    """
    out_dir = dest_dir if dest_dir is not None else (ROOT / ".claude-plugin")
    ...
    payload = {
        ...
        "description": (
            ...
            "Upstream's per-specialist marketplace (claude-for-financial-services with 20 plugins) "
            "is available directly from anthropics/financial-services."
        ),
        "_generated": (
            "GENERATED by scripts/sync-copilot.py - do not edit by hand. "
            "This file intentionally diverges from upstream anthropics/financial-services "
            "to expose the Copilot CLI umbrella plugin via repo-URL marketplace add."
        ),
        "plugins": [
            {
                "name": "financial-services",
                "displayName": "Claude for Financial Services (umbrella)",
                "source": "./copilot-cli/plugins/financial-services",
                "description": (
                    ...
                    "specialist - all in a single Copilot CLI plugin install. "
