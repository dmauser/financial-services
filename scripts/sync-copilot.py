...
def sync_copilot_marketplace(dest_dir: Path | None = None) -> int:
    """Write Copilot-dedicated marketplace.json files - one at the repo root
    (.copilot-plugin/marketplace.json) for repo-URL install, and one under
    copilot-cli/ (.copilot-plugin/marketplace.json) for local-clone install.
    Both register the same single umbrella `financial-services` plugin, kept
    separate from the upstream-tracked .claude-plugin/marketplace.json so
    upstream syncs from anthropics/financial-services never conflict on it.

    Repo-URL install (no clone needed):
    ...
    `dest_dir` overrides the output base. When set, both marketplace files are
    written under `dest_dir/<root|copilot-cli>/.copilot-plugin/marketplace.json`
    so check.py can diff them as a tree. Default: real repo paths.
    """
    if dest_dir is not None:
        root_out = dest_dir / "root" / ".copilot-plugin"
        copilot_out = dest_dir / "copilot-cli" / ".copilot-plugin"
    else:
        root_out = ROOT / ".copilot-plugin"
        copilot_out = COPILOT / ".copilot-plugin"
    ...
    written = 0
    for out_dir, source in (
        (root_out, "./copilot-cli/plugins/financial-services"),
        (copilot_out, "./plugins/financial-services"),
    ):
        out_dir.mkdir(parents=True, exist_ok=True)
        payload = {**common, "plugins": [{**plugin_entry, "source": source}]}
