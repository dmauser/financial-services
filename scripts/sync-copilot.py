...
def sync_root_marketplace() -> int:
    """Mirror copilot-cli/.copilot-plugin/marketplace.json to the repo-root
    .copilot-plugin/marketplace.json (with `source` paths rebased so they
    resolve against the repo root, where Copilot CLI looks for them when a
    user runs `/plugin marketplace add <owner>/<repo>`)."""
    src = COPILOT / ".copilot-plugin" / "marketplace.json"
    if not src.is_file():
        return 0
    ...
    for plugin in data.get("plugins", []):
        s = plugin.get("source", "")
        if s == "..":
            plugin["source"] = "copilot-cli"
        elif s.startswith("../"):
            plugin["source"] = "copilot-cli/" + s[3:]
    data["_generated"] = (
        "GENERATED by scripts/sync-copilot.py from "
        "copilot-cli/.copilot-plugin/marketplace.json - do not edit by hand"
    )
    dst_dir = ROOT / ".copilot-plugin"
    dst_dir.mkdir(parents=True, exist_ok=True)
    dst = dst_dir / "marketplace.json"
    payload = json.dumps(data, indent=2) + "\n"
    if not dst.is_file() or dst.read_text(encoding="utf-8") != payload:
