...
> This fork's [`/.claude-plugin/marketplace.json`](../.claude-plugin/marketplace.json) is **intentionally repurposed** as the Copilot CLI umbrella marketplace `financial-services-copilot`, registering one plugin: `financial-services` (the umbrella). Copilot CLI's `marketplace add <owner>/<repo>` only reads `.claude-plugin/`, so to deliver the umbrella over a repo URL we live in that file. The upstream per-specialist marketplace (`claude-for-financial-services` with 20 plugins) is unchanged at `anthropics/financial-services` — install from there if you need individual plugins.

```text
# 1. Register the marketplace (one-time)
copilot plugin marketplace add dmauser/financial-services

# 2. Install the umbrella (all 10 specialists + 66 skills + 39 slash commands + orchestrator)
...
The umbrella's source is at [`copilot-cli/plugins/financial-services/`](./plugins/financial-services/), generated by `scripts/sync-copilot.py` and drift-checked by `scripts/check.py`. Its orchestrator agent at `agents/financial-services.md` instructs the model to behave like Path A's `fs_capabilities` tool: list specialists/slash commands when asked, and route incoming requests to the right specialist. **Markdown-only** — no JS runtime tools; for in-process `fs_*` registration prefer Path A.

If you want the upstream per-specialist Claude plugins (financial-analysis, equity-research, pitch-agent, ...) install from upstream directly:
